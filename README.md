...
| 흐름 | **분기 누적(YTD)** | 분기보고서로 2025년 연중 모멘텀(YoY) 추적 ([`src/quarterly.py`](src/quarterly.py)) |
| 시각화 | **matplotlib** | 차트 9종 자동 생성 (`output/charts/`) |
...
python src/quarterly.py    # 2025 분기 흐름(YTD YoY) + 차트 9 생성
```
