-- 기업 재무·신용 분석 쿼리 모음
-- run_sql.py 가 "-- name:" 주석을 구분자로 각 쿼리를 실행/출력한다.
-- SQLD 역량 증명: 윈도우 함수(LAG, AVG OVER), CTE, CASE, 순위(RANK).

-- name: 1) 매출 추이와 전년대비 성장률 (YoY, 윈도우 함수)
SELECT
    corp_name AS 기업,
    bsns_year AS 연도,
...
-- name: 3) 안정성 지표 (부채비율 · 유동비율) — 여신심사 핵심
SELECT
    corp_name AS 기업,
    bsns_year AS 연도,
...
-- name: 5) 이자보상배율 (영업이익 / 이자비용) — 채무상환능력
SELECT
    corp_name AS 기업,
    bsns_year AS 연도,
    ROUND(operating_income / 1e8, 0)               AS 영업이익_억,
    ROUND(interest_expense / 1e8, 1)               AS 이자비용_억,
...
-- name: 6) 최신연도 3사 비교 + 종합 순위 (CTE + RANK)
WITH latest AS (
    SELECT f.*
    FROM financials f
    JOIN (SELECT corp_name, MAX(bsns_year) AS y FROM financials GROUP BY corp_name) m
